...
//        }
    ...
    HttpSession session = request.getSession();
        try {
            ...
            DbInsert addContact = new DbInsert();
            int rs = 0;
            out.print("contact_id" + request.getParameter("contact_id"));
            out.print("description" + request.getParameter("description"));
            rs = addContact.ReplyContact(request.getParameter("contact_id"), request.getParameter("description"));
            out.print(rs);
            if (rs == 1) {
                status = dbUpdate.UpdateReplyStatus(contact_id);
                response.sendRedirect("/Fortheby/admindisplayreply.jsp");
            } else {
                out.print("Error");
            }
        } catch (Exception e) {
