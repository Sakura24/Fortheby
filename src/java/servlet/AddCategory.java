...
@MultipartConfig(fileSizeThreshold = 1024 * 1024 * 2,
        ...
        maxRequestSize = 1024 * 1024 * 50)// it is used to get the image sie for the image
...
//        }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        //String filePath = "";
        try {
            DiskFileItemFactory factory = new DiskFileItemFactory(); // creating a new object of diskitemfactory
            ...
            while (collect.hasNext()) {
                ...
                if (item.isFormField()) {
                    ...
                    //out.print(fileName);
                    File file = new File("E:\\assignmentbackups\\Fortheby\\web\\upload\\" + fileName);// proviing the path to the file upload
                    item.write(file);
                }
            }
            pstm = con.prepareStatement("INSERT INTO categories (cat_name, image) VALUES (?,?)");// carry out insertion query
            pstm.setString(1, cat_name);
            pstm.setString(2, fileName);
            var = pstm.executeUpdate();
            out.print(var);
            if (var == 1) {
                response.sendRedirect("/Fortheby/admindisplaycategories.jsp");
            } else {
                out.print("Error");
            }
